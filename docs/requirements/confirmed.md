...
形式: YAML  
配置場所: `config/accman.yaml`（パスは環境変数 `ACCMAN_CONFIG` で上書き可能）  
...
ldap:
  ...
  port: 636
  tls: true
  ca_cert: /etc/ssl/certs/ca.pem   # 省略時はシステムの信頼ストアを使用

admin:
  bind_dn: "cn=admin,dc=example,dc=com"
  # パスワードは環境変数 ACCMAN_ADMIN_PASSWORD で渡す
...
| `ACCMAN_CONFIG` | 設定ファイルパス（省略時: `config/accman.yaml`） |
