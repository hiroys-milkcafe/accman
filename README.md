...
`rdn_attr` を `uid`、`[pam] pam_user` を `ou=people,dc=example,dc=com` とした場合、新規エントリのDNは `uid=<入力値>,ou=people,dc=example,dc=com` になる。
...
| `type` | 入力欄の種類 | `text` / `password` / `number` / `email` |
...
syslogへの追加出力を有効化している場合（`[log] syslog_enabled = true`）は、facility に応じたsyslogファイルも確認できる。rsyslogを使用する場合は `/etc/rsyslog.d/` に振り分けルールを設定する。
